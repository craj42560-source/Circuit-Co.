// ===== Bookings page — localStorage integration =====
...
function renderBookings() {
  ...
  if (!bookings || bookings.length === 0) {
    $container.html(
      '<div class="glass-strong p-5 text-center mx-auto animate-scale-in" style="max-width: 600px;">' +
        ...
        '<p style="color: var(--slate-500);" class="mb-4">When you book a repair, it will appear here. Your bookings are saved locally on this browser.</p>' +
        '<a href="contact.html" class="btn-primary-custom">Book your first repair</a>' +
      '</div>'
    );
    $('#filter-tabs').hide();
    $('#new-booking-cta').hide();
    return;
  }

  $('#filter-tabs').show();
  $('#new-booking-cta').show();
  ...
  var html = filtered.map(function (b) {
    ...
    // Safe service lookup
    var serviceName = b.service || 'Repair Service';
    if (typeof SERVICES !== 'undefined' && Array.isArray(SERVICES)) {
      var foundService = SERVICES.find(function (s) { return s.id === b.service || s.name === b.service; });
    ...
    // Safe urgency lookup
    var urgencyHtml = '';
    if (typeof URGENCY_OPTIONS !== 'undefined' && Array.isArray(URGENCY_OPTIONS)) {
      var urgencyOption = URGENCY_OPTIONS.find(function (u) { return u.id === b.urgency; });
      if (urgencyOption && urgencyOption.id !== 'standard') {
        urgencyHtml = '<span class="badge bg-primary-100" style="color: var(--primary-600); font-size: 0.7rem;">' + urgencyOption.label.split(' (')[0] + '</span>';
      }
    }

    var createdDate = b.createdAt ? new Date(b.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Recently';
        ...
        '<div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">' +
          '<div class="flex-grow-1">' +
            '<div class="d-flex align-items-center gap-2 mb-2">' +
              ...
              '<span class="small font-monospace" style="color: var(--slate-400);">' + (b.id || b.reference) + '</span>' +
            '</div>' +
            ...
            '<div class="d-flex flex-wrap gap-3 small" style="color: var(--slate-600);">' +
              '<span class="d-flex align-items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="6" y="2" width="12" height="20" rx="2"/><path d="M12 18h.01"/></svg> ' + (b.deviceType || b.device) + '</span>' +
            ...
            '</div>' +
            messageHtml +
            '<div class="d-flex flex-wrap gap-2 mt-2">' + priceHtml + urgencyHtml + promoHtml + '</div>' +
